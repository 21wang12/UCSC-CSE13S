...
//ref@tsp: same as example binary file with -h option.
void tsp_help(){
	printf("SYNOPSIS\n");
...
void dfs ( Graph *G, uint32_t v, Path *curr , Path * shortest , char * cities [], FILE * outfile ,bool verbose){
	...
	recursive ++;	//for verbose print
	
	//label v as visited and push it to curr path
	...
	if(path_vertices(curr) == graph_vertices(G)){
		//check if there have a direct connection from the last vertex to the first
		if(graph_has_edge(G,v,START_VERTEX)==false){	
			...
			if(path_length(curr)<path_length(shortest) || path_vertices(shortest)==0 ){
				//also check verbose print, to print all Hamiltonian path
				if(verbose){
	...
	//for all edges w adjacency with v, apply DFS to w. 
	...
	//label v as unvisited and pop it from curr path
	path_pop_vertex(curr,&temp_v,G);
